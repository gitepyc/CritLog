...
- **tweak:** Help panel credit line adds a "©" before "by Epyc" (in-game requested) - not pixel-verified, Classic Era's bundled fonts aren't guaranteed to have this glyph
- **tweak:** second in-game-screenshotted round on the panel-resize pass - Sound Settings' non-debug height bumped back up (400->460, Close was crowding the Aura/Death Sounds button row) and now also grows in *width* (not just height) in debug mode (520 vs. 460 base); Death Sounds height bumped back up a touch (390->420)
- **tweak:** in-game-screenshotted follow-up to the panel-resize pass - Roster Settings reverted to its original size (wasn't actually too big); Help panel narrowed further but taller again (640x640, was a too-wide-and-short 720x530); main Options panel, Death Sounds, and Roll Sounds all cut further (large empty space below their last row); Sound Settings now resizes dynamically between two fixed heights depending on whether the debug-only Raid Chat Phrases section is shown, instead of always reserving room for it
- **tweak:** most options panels shrunk (Sound Settings, Death Sounds, Roll Sounds, Aura Sounds, Help) - the shared Preview-button column offset (`UI/Shared.lua`'s `PREVIEW_COLUMN_X`) dropped from 340 to 300 and each panel's width/internal column offsets recalculated to match, instead of just shrinking outer dimensions (which would have overflowed the Preview-button column, a bug already fixed once before). Main options panel and the Highscore List popup left unchanged - both have prior in-game reports of being too narrow at a similar size. Not pixel-verified in-game
