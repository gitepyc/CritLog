...
-- Same reasoning as RECORD_ORDER, for the roster panel's three categories.
...
-- Lays out every row fresh on each call, same reasoning as
-- layoutHighscoreList: cheap (a roster this addon realistically deals with
-- is a couple dozen names at most), and avoids incrementally patching
-- anchors when a category's entry count changes between refreshes.
...
-- Editable copy of the melee/tank/priest death-sound name rosters
-- (CritLogDB.playerGroups, migrated once from CritLog.Data.playerGroups -
-- see Database.lua). Sized generously tall since a roster has no fixed
-- entry cap, unlike the highscore list popup.
local function buildRosterFrame()
    local f = createPanelFrame("CritLogRosterFrame", "CritLog Roster Settings", 440, 750)
    ...
    for _, kind in ipairs(ROSTER_ORDER) do
        ...
        categoryHeading:SetText(CritLog.Data.rosterKinds[kind].label)
